...
class UbtechASRProvider:
    ...
    def _run(self):
        while self.running:
            ...
            try:
            ...
            except requests.RequestException as e: # Specifically catch requests-related exceptions
                logging.error(f"UbtechASRProvider: RequestException during _get_single_utterance: {e}")
                # text remains None
            except Exception as e: # Catch any other unexpected errors
                logging.error(f"UbtechASRProvider: Unexpected error in _run's try block: {e}", exc_info=True)
                # text remains None
            finally:
                ...
                if text: # If we successfully got text
                    ...
                    logging.info(f"ASR got text: '{text}'. Pausing. Provider now sleeping for 2.0s to allow system processing.")
                    time.sleep(2.0) # MODIFIED: Add 2.0-second sleep while provider is paused
                elif not text: # No text obtained or error occurred
                    logging.debug("UbtechASRProvider: No text obtained or error occurred, sleeping briefly before allowing resume.")
    ...
    def _get_single_utterance(self) -> Optional[str]:
        ...
        try:
            ...
            if not self._start_voice_iat(ts):
                logging.warning("UbtechASRProvider: _start_voice_iat failed or returned False. ASR session not started.")
                return None
    ...
    def _stop_voice_iat(self):
        ...
        for attempt in range(max_retries):
            try:
                ...
                response = self.session.delete(f"{self.basic_url}voice/iat", timeout=2) # Use session
                response.raise_for_status() # Check for HTTP errors
                logging.debug("UbtechASRProvider: _stop_voice_iat request successful.")
                return # Success
            except requests.exceptions.HTTPError as e:
                if e.response.status_code >= 500 and attempt < max_retries - 1:
                    logging.warning(f"UbtechASRProvider: _stop_voice_iat failed with {e.response.status_code} (attempt {attempt + 1}), retrying in {retry_delay}s...")
                    time.sleep(retry_delay)
                    continue # Retry for 5xx errors
                else:
                    logging.error(f"UbtechASRProvider: _stop_voice_iat failed with HTTPError (attempt {attempt + 1}): {e}")
    ...
    def _get_voice_iat(self) -> Dict:
        ...
        for attempt in range(max_retries):
            try:
                ...
                if response_json.get("data") and isinstance(response_json["data"], str) and response_json.get("code") == 0:
                    ...
                    if cleaned_data_str:
                        try:
                            ...
                            logging.error(f"UbtechASRProvider: Failed to decode JSON from data string: '{cleaned_data_str}'")
                            # Keep original string data if decode fails, or handle as error
                return response_json # Success
            except requests.exceptions.HTTPError as e:
                if e.response.status_code >= 500 and attempt < max_retries - 1:
                    logging.warning(f"UbtechASRProvider: _get_voice_iat failed with {e.response.status_code} (attempt {attempt + 1}), retrying in {retry_delay}s...")
                    time.sleep(retry_delay)
                    continue # Retry for 5xx errors
                else:
                    logging.error(f"UbtechASRProvider: _get_voice_iat failed with HTTPError (attempt {attempt + 1}): {e}")
                    return {"code": -1, "message": str(e), "data": None, "status": "error"} # Return error structure
            except requests.RequestException as e:
                ...
                logging.error(f"UbtechASRProvider: _get_voice_iat failed to decode main JSON response (attempt {attempt + 1}): {e}")
                if attempt < max_retries - 1:
                    time.sleep(retry_delay)
                    continue # Retry for JSON decode errors
                return {"code": -1, "message": f"JSONDecodeError: {e}", "data": None, "status": "error"}
        
        logging.error(f"UbtechASRProvider: _get_voice_iat failed after {max_retries} retries.")
        return {"code": -1, "message": "Max retries exceeded", "data": None, "status": "error"} # Default error after all retries
...
class VLMGeminiProvider:
    ...
    VLM Provider that handles video streaming and Gemini API communication.
     ...
     fps : int
         Frames per second for the video stream. Default is 1 since Gemini has a rate limit.
    """

    def __init__(self, base_url: str, api_key: str, fps: int = 1):
        """
        ...
        logging.info("VLM Gemini Provider initialized")
                ...
                messages=[
                    {
                        "role": "user",
                        "content": [
                            {"type": "text", "text": "What is the most interesting aspect in this series of images?"},
                            {
                                "type": "image_url",
                                "image_url": {
    ...
    def start(self):
        ...
        self._thread.start()
        logging.info("VLM Gemini Provider started")

    def _run(self):
        """
        Main loop for the Gemini provider.

...
class LlamaLLM(LLM[R]):
    """
    A Meta Llama Language Learning Model implementation using Together AI.

    This class implements the LLM interface for Meta's Llama models through
    Together AI's API, handling configuration, authentication, and async API communication.
    ...
    def __init__(self, output_model: T.Type[R], config: LLMConfig = LLMConfig()):
        ...
        if not config.model:
            # Default to Llama 3.2 3B Instruct model on Together AI
            self._config.model = "meta-llama/Llama-3.2-3B-Instruct"

        # Initialize Together client for Llama models
        # Together AI provides access to various Llama models
    ...
    async def ask(
        ...
        try:
            ...
            for msg in messages:
                ...
                if role == "system":
                    # Llama models handle system messages differently
            ...
            # Call Together API with Llama model
            ...
            if response and hasattr(response, 'choices') and len(response.choices) > 0:
                ...
                logging.error("Empty response from Together API")
                return None
    ...
    def _supports_json_mode(self) -> bool:
        ...
        """
        # Together AI supports JSON mode for certain models
        json_capable_models = [
        ...
        return self._config.model in json_capable_models

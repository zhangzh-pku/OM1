...
* CycloneDDS (for DDS comms to the G1 motion computer/client)
...
Once you have all of those, then add the optional Python CycloneDDS module to OM1:
...
## Nvidia Orin System Description
...
```
pactl set-default-sink [SINK_NAME || SINK_ID] 
```

### Set default input device

```
pactl set-default-source [SOURCE_NAME || SOURCE_ID]
```
...
This is similar to boot from gantry, except, when you press "L1+UP", you have to help the humanoid stand up, while it straighten itself. For the sitdown procedure, back up the chair behind the robot, and select "L1+LEFT", and help the humanoid settle back into the chair. 
...
## Install CycloneDDS on Nvidia Orin

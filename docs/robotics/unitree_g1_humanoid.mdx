...
Install cycloneDDS via https://cyclonedds.io/docs/cyclonedds/latest/installation/installation.html. Please make sure to install `0.10.2`.

```bash
git clone https://github.com/eclipse-cyclonedds/cyclonedds.git --branch 0.10.2 --single-branch
cd cyclonedds
mkdir build
cd build
...
cd $HOME/Documents/GitHub/cyclonedds/install/share/CycloneDDS/examples/listtopicslisttopics
cmake .
cmake --build .
```

Then, set your wired etherent adapter to `192.168.123.99` and `255.255.255.0`, double check via `ifconfig`, and intert the correct etherent adapter name at the `NetworkInterface`:
...
From `cycloneDDS/build`, send some pings:

```bash ping
./bin/RoundtripPing 0 0 0
```

In another terminal, receive those pings and pong them back:
```bash pong
./bin/RoundtripPong
```
...
./bin/listtopics
```
For that to work, the `CYCLONEDDS_URI` needs to be set correctly. Set the ethernet adapter to the one that is connected to the robot. You can check this via `ifconfig`. 

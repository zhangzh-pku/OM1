...
When connecting the Arduino with a USB cable, you should see it appear as `/dev/ttyACM0` (sometimes the number can be different, for example `/dev/ttyACM1`). Simply run:

```bash
sudo dmesg | grep ttyACM*
```

and you should see it. At this point if you’re not sure which device is the Arduino board, run `sudo dmesg` and looks for entires with "Arduino" in it. This way you will easily spot the serial device name of your Arduino. Ypou can read the data with:

```bash
screen /dev/ttyACM0 115200
```

Note: on a typical TurtleBot4, the RPLIDAR is using `/dev/ttyUSB0` or `/dev/ttyUSB1`.

### Finding the Arduino on Mac

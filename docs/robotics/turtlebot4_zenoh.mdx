...
```bash
sudo apt install pulseaudio pulseaudio-utils pulseaudio-bluetooth ffmpeg
```

Reboot the turtlebot4. Set the default inputs. Unplug the onboard camera. Use the Logitech 270.   
...
trust <MAC> # for example, trust F8:5C:7D:DA:C9:75
pair <MAC>
connect <MAC>
```

If bluez is crashed/frozen, use `bluetoothctl` to run `power off`, `power on`, `scan on` to unstick. 

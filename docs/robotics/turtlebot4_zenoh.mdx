...
Suitable images can be installed via a [flashed SD card](https://turtlebot.github.io/turtlebot4-user-manual/setup/basic.html). Note that instuctions given are needlessly complex - basically, download the right image, flash a SD card with that image using your favorite tool/platform, insert the flashed SD card into the RPi, and turn it on.
...
NOTE: The TurtleBot4 contains two entirely seperate computers, the Raspberry Pi (RPi) and the Create3. These computers are accessed in different ways, run different software, and have different roles. 

NOTE: Please make sure you have updated the Raspberry Pi's OS (use `sudo apt update && sudo apt upgrade`).

NOTE: Please make sure you have updated the TurtleBot4's internal Create3 computer to `iRobot® Create® 3 Release H.2.6` via the Create3's html "Update" page.

NOTE: We now assume you are generally familiar with the ROS2/Turtlebot4 ecosystem, including ssh-ing to the Raspberry PI command line, the use of the `turtlebot4-setup` program, and interacting with the internal Create3 via its  webpage at IP:8080. 

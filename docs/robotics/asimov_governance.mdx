...
We leverage a blockchain-based system for regulating robot bahaviors. This is done via storing rule sets for desired robot behaviors on smart contracts following ERC-7777 contract standards, and inserting those rules to the prompts of our models. Such a system affords immutability and transparency to the rules that robots need to follow, and help ensure that the robots observe safe and human-friendly behaivors. 

For a full explanation of the smart contract implementation, check https://eips.ethereum.org/EIPS/eip-7777 and https://openmind.org/research.html. The implementation details for the current governance rules based on Asimov's Laws.

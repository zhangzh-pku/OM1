...
- `sensor_mounting_angle`: The `sensor_mounting_angle` refers to the angle between the LIDAR sensor’s scanning plane and the horizontal plane of your robot or platform. This parameter is crucial for accurately interpreting the sensor’s data within your system’s coordinate framework.  
  - **NOTE**: The `sensor_mounting_angle` can be read physically from the LIDAR. Refer to the following image for guidance:  
    ![sensor_mounting_angle](/assets/lidar_angle.png)  

- `angles_blanked`: The `angles_blanked` parameter isn’t a standard feature in the official RPLIDAR documentation or SDK. However, in various applications and software integrations, it often refers to specific angular sectors where LIDAR data is intentionally ignored or filtered out.  
## Unitree RPLidar

...
OM1 can control a Unitree Go2 EDU. This has been tested on Nvidia Orin, Mac Mini, and current (silicon) Mac laptops.
...
Set `CYCLONEDDS_HOME`, `CMAKE_PREFIX_PATH`, and `CYCLONEDDS_URI` to the correct values, shown conceptually below. You should add these paths to your environment via your `.zshrc` or equivalent.

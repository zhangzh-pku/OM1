...
OM1 can control a Unitree Go2 EDU. This has been tested on a Mac laptop running Sequoia 15.2.

<Steps>

<Step title="Step 1 - Game controller">
Connect a Bluetooth `XBOX` controller to your computer.
...
Set `CYCLONEDDS_HOME`, `CMAKE_PREFIX_PATH`, and `CYCLONEDDS_URI`. You should add these paths to your environment via your `.zshrc` or equivalent.
...
Then, compile the `listtopics` example:
...
This should result in an extensive data dump of available topics:
...
```bash
export PYTHONPATH="$HOME/Documents/GitHub/OM1/src/:$PYTHONPATH"
```

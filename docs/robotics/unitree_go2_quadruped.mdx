...
In this configuration, the quadruped observes its environment, listens and speaks, but there is no AI-controlled movement. However, you can manually control the dog's movements with an Xbox controller. 
...
OM1 will provide LIDAR and other data to a system of LLMs, allowing them to autonomously explore indoor and outdoor environments. You can also manually control the dog via the Xbox game controller. Press:

* A to stand up
* B to sit down
* The D-pad allows you to steer the quadruped. 

The Xbox game controller has command precedence over the AI, so game controller inputs will block AI-generated physical actions.  

...
```
Set the correct `CYCLONEDDS_HOME` env var. This is where the actual CycloneDDS is installed on your computer, and has nothing to do with the Python libraries:
```bash
export CYCLONEDDS_HOME="$HOME/unitree_ros2/cyclonedds_ws/install/cyclonedds"
```
If you do not do this correctly, installation of the Python CycloneDDS will fail since it cannot find the correct libraries. 

Note: make absolutely sure `CYCLONEDDS_HOME` actually points to the CycloneDDS install /lib.
...
Note: Install `v4l-utils` to help debug video problems:
```bash
sudo apt-get update
sudo apt-get install v4l-utils
```

**WARNING**: The camera system, if not correctly configured, has a tendency to bring down the entire USB bus. FIX: reboot the humanoid.

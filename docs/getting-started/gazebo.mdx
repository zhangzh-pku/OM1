...
The Gazebo simulator is an industry standard simulator for robots. The robot shape/size is specifies via `.dae` files, and those robots can then navigate a defined digital works. The simulation framework is useful for prototyping, debugging, and reinforcement learning. 
...
./gazebo/macOS.sh
```
The script also checks and installs Gazebo Harmonic https://gazebosim.org/docs/harmonic/getstarted/ if not already installed. If this is your first time installing Gazebo, grab a coffee, since this will take a while.  

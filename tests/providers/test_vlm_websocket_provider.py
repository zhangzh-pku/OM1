...
from providers.vlm_websocket_provider import VLMWebSocketProvider
...
def reset_singleton():
    VLMWebSocketProvider._instance = None
    yield
...
def mock_dependencies():
    with (
        patch("providers.vlm_websocket_provider.ws.Client") as mock_ws_client,
        patch("providers.vlm_websocket_provider.VideoStream") as mock_video_stream,
    ):
...
def test_initialization(ws_url, fps, mock_dependencies):
    ...
    provider = VLMWebSocketProvider(ws_url, fps=fps)
...
def test_singleton_pattern(ws_url, fps):
    provider1 = VLMWebSocketProvider(ws_url, fps=fps)
    provider2 = VLMWebSocketProvider(ws_url, fps=fps)
